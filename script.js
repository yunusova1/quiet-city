...
// ============ SUPABASE НАСТРОЙКИ ============
// ⚠️ ВСТАВЬТЕ СВОИ ДАННЫЕ ИЗ НАСТРОЕК API (Settings → API):
const SUPABASE_URL = 'https://efddiibhwwyonepsohww.supabase.co';   // Ваш Project URL
const SUPABASE_ANON_KEY = 'sb_publishable_NrRgpL7bFf5WFgr4YW7UtQ_QSkIChxJ';        // Ваш anon public ключ
...
// Загрузка всех замеров из облака
async function loadFromCloud() {
    try {
        const data = await supabaseFetch('measurements?order=id.desc');
        return data || [];
    } catch (e) {
...
// Очистка таблицы в облаке
async function clearCloud() {
    try {
        await supabaseFetch('measurements', {
            method: 'DELETE',
            headers: { 'Prefer': 'return=minimal' }
        });
    } catch (e) {
        console.error('❌ Ошибка очистки облака:', e);
    }
}

// Сохранение данных в облако (массово)
async function saveToCloud(measurements) {
    try {
        for (const item of measurements) {
            const { id, ...clean } = item; // удаляем локальный id
            await supabaseFetch('measurements', {
                method: 'POST',
...
async function syncWithCloud(showMessage = true) {
    ...
    try {
        ...
        if (cloudData === null) {
            if (showMessage) alert('❌ Ошибка загрузки из облака! Проверьте интернет и ключи.');
            if (syncBtn) { syncBtn.innerHTML = syncBtn.dataset.originalText || '☁️ Синхронизация'; syncBtn.disabled = false; }
            isSyncing = false;
            return;
        }

        // Локальные данные
        const localData = await getAllMeasurements();

        let finalData = [];
        let source = '';

        if (cloudData.length === 0 && localData.length === 0) {
            if (showMessage) alert('📭 Нет данных для синхронизации');
            if (syncBtn) { syncBtn.innerHTML = syncBtn.dataset.originalText || '☁️ Синхронизация'; syncBtn.disabled = false; }
            isSyncing = false;
            return;
        }
        ...
        if (cloudData.length === 0 && localData.length > 0) {
            finalData = localData;
            source = 'локальные → облако';
        } else if (cloudData.length > 0 && localData.length === 0) {
            finalData = cloudData;
            source = 'облако → локальные';
        } else {
            // Объединяем без дубликатов по timestamp
            const all = [...cloudData, ...localData];
            const unique = new Map();
            all.forEach(item => {
                const key = `${item.timestamp}_${item.lat.toFixed(6)}_${item.lng.toFixed(6)}`;
                if (!unique.has(key)) unique.set(key, item);
            });
            finalData = Array.from(unique.values());
            source = `объединено (${cloudData.length} + ${localData.length} = ${finalData.length})`;
        }

        // Сохраняем локально (очищаем и заново)
        ...
        // Очищаем облако и загружаем объединённые данные
        await clearCloud();
        const saved = await saveToCloud(finalData);

        if (saved) {
            if (showMessage) alert(`✅ Синхронизация завершена!\n📊 ${finalData.length} замеров\n🔄 ${source}`);
            await applyFilters();
            renderComments();
            renderHistoryPanel();
        } else {
            if (showMessage) alert('⚠️ Данные сохранены локально, но не в облаке! Проверьте интернет.');
        }

    } catch (e) {
        console.error('❌ Ошибка синхронизации:', e);
        if (showMessage) alert('❌ Ошибка синхронизации: ' + e.message);
    }

    if (syncBtn) {
        syncBtn.innerHTML = syncBtn.dataset.originalText || '☁️ Синхронизация';
...
window.addCommentToPoint = async function(id) {
    ...
    if (measurement) {
        ...
        // Авто-синхронизация после добавления комментария
        if (navigator.onLine) {
...
async function saveCurrentMeasurement() {
    ...
    try {
        ...
        // Авто-синхронизация после сохранения
        if (navigator.onLine) {
...
async function init() {
    ...
    // Авто-синхронизация при загрузке (если есть интернет)
    if (navigator.onLine) {
    ...
    // Кнопка синхронизации (добавьте её в HTML)
    const syncBtn = document.getElementById('syncBtn');
    if (syncBtn) {
        syncBtn.dataset.originalText = syncBtn.innerHTML;
        syncBtn.onclick = () => syncWithCloud(true);
    }
    
    // Доп. перерисовка карты после загрузки
    setTimeout(() => map.invalidateSize(), 500);
    
    console.log("✅ Приложение готово! Синхронизация с Supabase включена.");
}
