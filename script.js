...
// ============ НАСТРОЙКА FIREBASE ============
// ⚠️ Если ваши ключи отличаются — замените их на свои.
...
async function saveCurrentMeasurement() {
    ...
    try {
        ...
        // onSnapshot обновит карту автоматически
...
async function exportToFile() {
    ...
    alert(`📁 Данные экспортированы!\nВсего замеров: ${allMeasurements.length}`);
}
...
async function init() {
    initMap();
    subscribeToMeasurements(); // подписка на реальное время
    setupSearch();
    setupViewSwitcher();
    setupFilters();
    setupCalendar();
