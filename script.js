...
let statsChartMode = 'histogram';

// Флаг, что Firebase уже загружен (чтобы не дублировать подписку)
...
function openStatsModal() {
    ...
    modal.style.display = 'flex';
    // Если замеров нет – показываем демо-данные, чтобы график был виден
    if (allMeasurements.length === 0) {
        // Добавляем тестовые данные в allMeasurements (только для отображения, не сохраняем)
        const demo = [
            { db: 42, date: '2026-06-28', timestamp: Date.now() - 86400000 },
            { db: 57, date: '2026-06-29', timestamp: Date.now() - 3600000 },
            { db: 73, date: '2026-06-29', timestamp: Date.now() }
        ];
...
function renderChart(data) {
    ...
    if (statsChartMode === 'histogram') {
        ...
        const dbValues = sorted.map(m => m.db);
        statsChartInstance = new Chart(ctx, {
            type: 'line',
            data: {
                labels: dates,
            ...
            options: {
                ...
                plugins: {
                    legend: { display: true },
                    title: { display: true, text: 'Динамика замеров по датам' }
                },
                scales: {
                    y: {
                        beginAtZero: true,
...
async function init() {
    ...
    document.getElementById('statsHistogramBtn').addEventListener('click', () => {
        statsChartMode = 'histogram';
        buildStats();
    });
    document.getElementById('statsLineBtn').addEventListener('click', () => {
        statsChartMode = 'line';
        buildStats();
    });
    ...
    // Если после загрузки замеров нет – всё равно даём возможность увидеть демо-график
    setTimeout(() => {
        if (allMeasurements.length === 0) {
            console.log('📊 Нет замеров. Статистика покажет демо-данные при открытии.');
        }
    }, 1000);
